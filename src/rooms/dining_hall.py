...
def handle_dining_hall_loot(game):
    ...
    if game.dining_hall_looted:
        game.game_text = format_output("You've already searched the dining hall.")
        game.render_screen()
        prompt_continue()
        return

    game.game_text = format_output("You search the long dining tables and cabinets...")
    game.render_screen()
    prompt_continue()

    loot_items = get_dining_hall_loot()
    if not loot_items:
        game.game_text = format_output("You find nothing of value.")
        game.render_screen()
        prompt_continue()
